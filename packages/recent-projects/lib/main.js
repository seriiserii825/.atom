...
var destroyViews = function destroyViews() {
	...
	for (var i = 0, len = ref.length; i < len; i++) {
		...
		for (var j = 0, len1 = ref1.length; j < len1; j++) {
			var item = ref1[j];
			if (_instanceof(item, RecentProjectsView)) {
...
var closeDefaultBuffer = function closeDefaultBuffer() {
	var ref;
	var cancelSub = false;
...
var viewOpener = function viewOpener(filePath) {
	if (filePath === projectsHomepageUri) {
		return createView();
	}
};

var processProject = function processProject(projectPaths) {
...
module.exports = {
	...
	activate: function activate() {
		...
		this.subs.add(atom.commands.add('atom-workspace', {
			'recent-projects:open': function recentProjectsOpen() {
